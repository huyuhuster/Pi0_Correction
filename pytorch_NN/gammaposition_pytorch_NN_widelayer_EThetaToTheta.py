...
class Net(nn.Module):
    def __init__(self, n_feature,  n_output):
        ...
        self.hidden1 = torch.nn.Linear(n_feature, 2048)   # define hiden layer, liner out put
        # self.drop1   = torch.nn.Dropout(0.5)
        self.hidden2 = torch.nn.Linear(2048, 2048)   # define hiden layer, liner out put
        ...
        self.predict = torch.nn.Linear(2048, n_output)   # define output layer, liner out put
...
def train(h5file, h5key, pklfile, validationh5, trainedlossplot, train_target, train_lossh5):
        ...
        net = Net(n_feature=50,  n_output=1)
        ...
        #print(mydf_test.iloc[:,4:54].head())
        test_data_np = mydf_test.iloc[:,4:54].replace(np.nan, 0.0).values
        ...
        for epoch in range(EPOCH):
           ...
           for mydf_readd5 in  reader: 	
              ...
              # ****** train dataset
              train_data_np = mydf_train.iloc[:,4:54].replace(np.nan, 0.0).values
