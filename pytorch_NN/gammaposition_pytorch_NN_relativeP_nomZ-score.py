...
def train(h5file, h5key, pklfile, validationh5, trainedlossplot, train_target, train_lossh5):
        ...
        for epoch in range(EPOCH):
           ...
           for mydf_readd5 in  reader: 	
              ...
              #print("train_data_np: ", mydf_train.iloc[:,-25:][0:10])
...
#	reader = pd.read_hdf(h5file, h5key_test, chunksize=100000)
	...
	for mydf_test in reader:
		...
		if train_target == 'phi':
		   ...
		   pred_df['mcPhi_c'] = pred_df['mcPhi_rel']  * mydf_test['phi_UD'] + mydf_test[ 'phi_central']
		   pred_df['phi_c']   = pred_df['phi_rel']    * mydf_test['phi_UD'] + mydf_test[ 'phi_central']
		   pred_df['prePhi']  = pred_df['prePhi_rel'] * mydf_test['phi_UD'] + mydf_test[ 'phi_central']
		   pred_df['mcPhi_c'][pred_df['mcPhi_c'] > 3.1415926] = pred_df['mcPhi_c'] - 3.1415926*2
		   pred_df['phi_c']  [pred_df['phi_c']   > 3.1415926] = pred_df['phi_c']   - 3.1415926*2
		   pred_df['prePhi'] [pred_df['prePhi']  > 3.1415926] = pred_df['prePhi']  - 3.1415926*2
		   pred_df['mcPhi_c'][pred_df['mcPhi_c'] <-3.1415926] = pred_df['mcPhi_c'] + 3.1415926*2
		   pred_df['phi_c']  [pred_df['phi_c']   <-3.1415926] = pred_df['phi_c']   + 3.1415926*2
		   pred_df['prePhi'] [pred_df['prePhi']  <-3.1415926] = pred_df['prePhi']  + 3.1415926*2
		elif train_target == 'theta':
...
phase = '' # '_phase3'	
trainh5file = 'B2APi0selection_' + ID_train + '_crystal_addmcMatchWeight_modified_threshold' + str(threshold_train)  + '_rel_Z-score.h5'
testh5file  = 'B2APi0selection_' + ID_test  + '_crystal_addmcMatchWeight_modified_threshold' + str(threshold) + phase  + '_rel_Z-score.h5'
